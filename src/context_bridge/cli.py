...
def init(ctx):
    ...
    console.print(f"   Memories: {stats['total_memories']} | Sessions: {stats['total_sessions']}")
...
def index(ctx, path):
    ...
    store.close()
    console.print(f"[green]✅ Indexed {total_sessions} sessions from {len(files)} files[/green]")
...
def recall(ctx, query_text, agent, limit):
    ...
    for m in results:
        ...
        table.add_row(m.memory_type.value, m.source_agent, content_preview, f"{m.importance:.2f}")
...
@click.option("--type", "mtype", type=click.Choice(["decision", "pattern", "lesson", "preference", "architecture"]), default="decision")
@click.pass_context
...
def stats(ctx):
    ...
    s = store.get_stats()
    store.close()
    console.print(f"[bold]Context Bridge Stats[/bold]")
